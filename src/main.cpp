...
const string strMessageMagic = "Stasis Signed Message:\n";
...
string getDevAddress(int nHeight) {
	...
	if (addrId == 0) {
        return "SaJUST3UEhbSoKEXazRKEBC85gLfj7TcpU";
	} else if (addrId == 1) {
        return "STQEe8hHybYMcgcoTyUP7ALsgQVFt8ASQx";
	} else if (addrId == 2) {
        return "SUV46am3c414WcTcvmk3gUoKjuWm4AKcoJ";
	} else {
        return "SkUuFpQMhpBmnDXSZ3oSQyeZyUaixa9ngk";
	}
}
        ...
        if(MasternodePayments)
        {
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    //if (pindex->nHeight > 155000 && pindex->nHeight < 175000) { changed
                    if (pindex->nHeight > 500 && pindex->nHeight < 1000) {
                        int64_t blockValue = vtx[1].vout[1].nValue;
                        int64_t nCredit = blockValue / 0.9;
                        int64_t devFee = nCredit * 0.1 - 0.5;
                        CStasisAddress devRewardAddress(getDevAddress(pindex->nHeight + 1));
                        ...
                        if(lastBlockTx.nValue < devFee || lastBlockTx.scriptPubKey != devRewardscriptPubKey)
                            foundDevFee = false;
                    //} else if (pindex->nHeight >= 175000) { changed
                    } else if (pindex->nHeight >= 1000) {
                        CStasisAddress devRewardAddress(getDevAddress(pindex->nHeight + 1));
                        CScript devRewardscriptPubKey = GetScriptForDestination(devRewardAddress.Get());
                        foundDevFee = false;
                        for (unsigned int i = 0; i < vtx[1].vout.size(); i++) {

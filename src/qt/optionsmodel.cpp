...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeStasisAmount"))
        settings.setValue("nAnonymizeStasisAmount", 1000);
    nAnonymizeStasisAmount = settings.value("nAnonymizeStasisAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeStasisAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeStasisAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeStasisAmount:
            return QVariant(nAnonymizeStasisAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeStasisAmount:
            nAnonymizeStasisAmount = value.toInt();
            settings.setValue("nAnonymizeStasisAmount", nAnonymizeStasisAmount);
            emit AnonymizeStasisAmountChanged(nAnonymizeStasisAmount);
            break;
        default:
            break;
        }

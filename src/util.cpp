...
int nAnonymizeStasisAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Stasis
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Stasis
    // Mac: ~/Library/Application Support/Stasis
    // Unix: ~/.Stasis
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Stasis";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Stasis.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Stasisd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Stasis will not work properly.");

...
//
// Bootup the masternode, look for a 175000 SAS input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Stasis.conf */
...
{
    CStasisAddress address(collateralAddress);

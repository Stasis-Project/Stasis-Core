...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeStasisAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeStasisAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeStasisAmount = strAnonymizeStasisAmount.remove(strAnonymizeStasisAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStasisAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeStasisAmount*COIN) nMaxToAnonymize = nAnonymizeStasisAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeStasisAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeStasisAmount));
        strAnonymizeStasisAmount = strAnonymizeStasisAmount.remove(strAnonymizeStasisAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStasisAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {

...
namespace {
    class CStasisAddressVisitor : public boost::static_visitor<bool> {
    private:
        CStasisAddress *addr;
    public:
        CStasisAddressVisitor(CStasisAddress *addrIn) : addr(addrIn) { }
...
bool CStasisAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CStasisAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CStasisAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CStasisAddressVisitor(this), dest);
}

bool CStasisAddress::IsValid() const {
...
CTxDestination CStasisAddress::Get() const {
    if (!IsValid())
...
bool CStasisAddress::GetKeyID(CKeyID &keyID) const {
...
bool CStasisAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CStasisSecret::SetKey(const CKey& vchSecret) {
...
CKey CStasisSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CStasisSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CStasisSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CStasisSecret::SetString(const std::string& strSecret) {
